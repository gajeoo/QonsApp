...
export const create = mutation({
  ...
  handler: async (ctx, { email, role }) => {
    ...
    if (profile.role === "worker") return { success: false, error: "Workers cannot send invitations" };
...
export const listAll = query({
  ...
  handler: async (ctx) => {
    ...
    const invites = await ctx.db.query("invitations").order("desc").collect();
...
export const acceptInvitation = mutation({
  ...
  handler: async (ctx, { token }) => {
    ...
    if (invite.status !== "pending") return { success: false, error: "Invitation already used" };
    if (invite.expiresAt < Date.now()) return { success: false, error: "Invitation expired" };
