...
export const getStats = query({
  ...
  handler: async (ctx) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) return { total: 0, active: 0, onLeave: 0, roles: {} as Record<string, number> };
...
export const create = mutation({
  args: {
    ...
    role: v.union(v.literal("concierge"), v.literal("porter"), v.literal("supervisor"), v.literal("manager"), v.literal("maintenance")),
    hourlyRate: v.number(),
...
export const update = mutation({
  args: {
    ...
    phone: v.optional(v.string()),
    role: v.optional(v.union(v.literal("concierge"), v.literal("porter"), v.literal("supervisor"), v.literal("manager"), v.literal("maintenance"))),
