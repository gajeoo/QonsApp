...
> = {
  new: { label: "New", color: "bg-chart-3/10 text-chart-3 border-chart-3/30", icon: UserPlus },
  contacted: { label: "Contacted", color: "bg-chart-2/10 text-chart-2 border-chart-2/30", icon: Mail },
  converted: { label: "Converted", color: "bg-teal/10 text-teal border-teal/30", icon: Check },
  archived: { label: "Archived", color: "bg-muted text-muted-foreground border-border", icon: X },
};
...
export function AdminLeadsPage() {
  ...
  const leads = useQuery(
    api.leads.list,
    filterStatus
      ? { status: filterStatus as "new" | "contacted" | "converted" | "archived" }
      : {},
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        {[
          { label: "Total", value: leadStats?.total ?? 0, color: "text-foreground" },
          { label: "New", value: leadStats?.new ?? 0, color: "text-chart-3" },
          { label: "Contacted", value: leadStats?.contacted ?? 0, color: "text-chart-2" },
          { label: "Converted", value: leadStats?.converted ?? 0, color: "text-teal" },
        ].map((s) => (
          <Card key={s.label}>
      ...
      <div className="space-y-3">
        {filteredLeads === undefined ? (
          ...
          filteredLeads.map((lead) => {
            ...
            return (
              <Card key={lead._id} className="hover:shadow-sm transition-shadow">
