...
// Input unused but kept for future use
...
function TimeTrackingPageInner() {
  ...
  const handleClockIn = async () => {
    if (!clockInForm.staffId || !clockInForm.propertyId) { toast.error("Select staff and property"); return; }
    setLoading("clock-in");
    try {
      ...
      await clockInMut({
        staffId: clockInForm.staffId as Id<"staff">,
        propertyId: clockInForm.propertyId as Id<"properties">,
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">GPS-verified clock-in and time tracking</p>
        </div>
            ...
            {activeEntries.map((entry) => (
              <Card key={entry._id} className="border-green-200 bg-green-50/50">
                <CardContent className="p-4 flex items-center justify-between">
                  <div>
                    <p className="font-semibold">{getStaffName(entry.staffId)}</p>
      ...
      <div>
        ...
        {timeEntries.length === 0 ? (
          <Card><CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground">No time entries yet. Clock in a staff member to start tracking.</p>
          </CardContent></Card>
        ) : (
              ...
              <Card key={entry._id}>
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div>
                      <p className="font-medium">{getStaffName(entry.staffId)}</p>
                      <p className="text-sm text-muted-foreground">{getPropertyName(entry.propertyId)}</p>
                    </div>
                  </div>
