    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Schedule and manage shifts across all properties</p>
        </div>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Total Shifts", value: stats?.total ?? 0, icon: Calendar },
          { label: "Scheduled", value: stats?.scheduled ?? 0, icon: Clock },
          { label: "Open (Unfilled)", value: stats?.open ?? 0, icon: AlertTriangle },
            ...
            return (
              <div key={day.date} className={`min-h-[200px] border rounded-lg p-2 ${day.isToday ? "border-teal bg-teal/5" : ""}`}>
                ...
                <div className="space-y-1.5">
                  {dayShifts.map((s) => (
                    <div key={s._id} className={`text-[11px] p-1.5 rounded cursor-pointer hover:opacity-80 ${s.aiAssigned ? "border-l-2 border-teal" : ""} ${statusColor[s.status]?.replace("text-", "").includes("amber") ? "bg-amber-50 border border-amber-200" : "bg-blue-50 border border-blue-200"}`}>
                      <DropdownMenu>
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{getStaffName(s.staffId) || "Unassigned"}</p>
                  </div>
